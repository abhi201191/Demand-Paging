...
public class LRU extends PageReplacement {
	...
	public void allocateFrame(List<Process> processList,
		...
		while (itr.hasNext()) {
			...
			System.out.println("\nProcess " + processIDnew + "\nLogical Address <" + ref.getLogicalAddress() + ">");
			...
			for (int i = 0; i < sim.sizeOfTLB; i++) {
				if (TLB[i][0] == pageNo) {
					...
					/* Remove entry of Page number from queue and put it at back */
					fifoQueue.remove(fifoQueue.indexOf(pageNo));
			...
			if (presentInTLB != true) {
				...
				if (PTE.getValidBit() == 1) {
					...
					/*
					 * Page referenced should be removed from fifoQueue and add
					 * to last so that that cannot be choose as victim frame as
					 * prior to other which is already present in fifoQueue.
					 */
					fifoQueue.remove(fifoQueue.indexOf(pageNo));
					fifoQueue.add(pageNo);
				}
				 ...
				 */
					 ...
					 */
						...
						/* Remove first element from fifoQueue as victim */
						victimFrame = fifoQueue.remove(0);
				...
				/*
				 * Page which is reference, then its entry should be present in
				 * TLB
				 */
				...
				/* TLB is full then choose victim entry */
					...
					/*
					 * If TLB entry for dirty bit for victim is set, then set
					 * dirty bit for corresponding page no in page table
					 */
					if (TLB[rowNo][2] == 1)
