...
public class ApproximationLRU extends PageReplacement {

	/* Function to return victim frame */
	public int getVictimFrame(List<Integer> fifoQueue, PageTable PTable)
	{
		/* 
		 * Select first frame from fifoQueue as victim frame 
		 * and store value of its reference byte in victimFrameRefByteValue.
		 */
		int victimFrameRefByteValue = (int) PTable.PT.get(fifoQueue.get(0)).getReferenceByte();
		int victimFrame = fifoQueue.get(0);
		
		/* 
		 * Find page whose reference byte value is minimum from remaining pages 
		 * and select that page as victim frame.
		 */
		for(int i = 1; i < fifoQueue.size(); i++)
		{
			if(victimFrameRefByteValue > (int) PTable.PT.get(fifoQueue.get(i)).getReferenceByte())
			{
				victimFrameRefByteValue = (int) PTable.PT.get(fifoQueue.get(i)).getReferenceByte();
	...
	/* 
	 * Function to shift reference byte by one from left to right 
	 * and store reference bit to most significant bit of reference byte.
	 */
	public void shiftRefByteByOne(PageTable PTable)
	{
		/* Iterate over all page table entries */
		for(int i = 0; i < PTable.PT.size(); i++)
		{
			/* Shift reference byte by one to left */
			char c = (char)(PTable.PT.get(i).getReferenceByte() >> 1);
			
			/* 
			 * If reference bit is 1 then store this reference bit 
			 * to most significant bit of reference byte by ORing 10000000 with reference Byte.
			 */
			if(PTable.PT.get(i).getReferenceBit() == 1)
			{
				 c = (char)(c | 0x80);
				 PTable.PT.get(i).setReferenceBit(0);
			}
			
			/* Set new reference byte */
			PTable.PT.get(i).setReferenceByte(c);
		}
	}
	...
	public void allocateFrame(List<Process> processList,
		...
		while (itr.hasNext()) {
			...
			/* Function call to shift reference byte of each page entry for each process periodically */
			triggerCount++;
			if(triggerCount == 3)
			{
				for(int k = 0; k < processList.size(); k++)
					shiftRefByteByOne(processList.get(k).getPageTable());
				triggerCount = 0;
			}
		}
	}
